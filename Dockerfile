...
EXPOSE 10000
CMD ["sh", "-lc", "if [ -z \"${APP_URL:-}\" ] && [ -n \"${RENDER_EXTERNAL_URL:-}\" ]; then export APP_URL=\"$RENDER_EXTERNAL_URL\"; fi && php artisan optimize:clear && php artisan migrate --force && php artisan db:seed --class=DatabaseSeeder --force && exec php artisan serve --host=0.0.0.0 --port=${PORT:-10000}"]
